 ...
 *
 * Exposes a transcript array (like useDeepDiveCall) so the dialog can render
 * the same toggleable transcript view the guide deep-dive uses.
 */
...
export type UseJobVoiceCallArgs = {
  jobPostingId: Id<"job_postings">;
  voiceId?: string;
};

export type UseJobVoiceCallReturn = {
  callState: JobCallState;
  error: string | null;
  isAITalking: boolean;
...
export function useJobVoiceCall(
  args: UseJobVoiceCallArgs,
): UseJobVoiceCallReturn {
  const { jobPostingId, voiceId } = args;

  const mintSession = useAction(api.jobVoiceNode.mintJobSession);
  const appendMessage = useMutation(api.voiceCalls.appendMessage);
  const finalize = useMutation(api.voiceCalls.finalize);

  const [callState, setCallState] = useState<JobCallState>("idle");
  const [error, setError] = useState<string | null>(null);
  const [isAITalking, setIsAITalking] = useState(false);
  const [userSpeaking, setUserSpeaking] = useState(false);
  const [transcript, setTranscript] = useState<JobTranscriptEntry[]>([]);
  ...
  const persistMessage = useCallback(
    async (message: AppendMessageInput) => {
      ...
      try {
        ...
        console.warn("useJobVoiceCall: appendMessage failed", err);
      }
    },
    [appendMessage],
  );
    ...
    async (status: "completed" | "interrupted" | "error") => {
      ...
      if (sessionId) {
        try {
          ...
          console.warn("useJobVoiceCall: finalize failed", err);
        }
      }
  ...
  const startCall = useCallback(async () => {
    ...
    try {
      mintResult = await mintSession({ jobPostingId, voiceId });
    } catch (err) {
      setError(
      ...
      try {
        ...
        if ("setupComplete" in data && !captureRef.current) {
          try {
            ...
            console.error("useJobVoiceCall: mic capture failed", err);
            setError(
        ...
        console.warn("useJobVoiceCall: message parse error", err);
      }
    };
...
function reasonToMessage(
  ...
  switch (reason) {
    case "anonymous":
      return "Sign in to talk through this role.";
    case "posting-not-found":
      return "Couldn't load this job posting. Try refreshing.";
    case "posting-not-ready":
      return "We're still preparing this listing. Try again in a minute.";
    case "no-api-key":
